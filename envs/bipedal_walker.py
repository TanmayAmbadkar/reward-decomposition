...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self, render_mode: Optional[str] = None, hardcore: bool = False, scalar_reward: bool = False):
    ...
    def step(self, action: np.ndarray):
        ...
        if self.prev_shaping is not None:
            reward.extend([x - y for x, y in zip(shaping, self.prev_shaping)])
        else:
            reward.extend([0]*2)
        self.prev_shaping = shaping

        for a in action:
            reward.append(-0.00035 * MOTORS_TORQUE * np.clip(np.abs(a), 0, 1))
...
def demo_heuristic_walker(env, seed=None, render=False):
    total_reward = [0]*8
    steps = 0
    ...
    while True:
        ...
        total_reward = [x + y for x, y in zip(total_reward, r)]
        rewards.append(r)
