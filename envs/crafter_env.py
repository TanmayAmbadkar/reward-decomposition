...
class CrafterEnv(BaseClass):

  def __init__(
      self, area=(64, 64), view=(9, 9), size=(64, 64),
      reward=True, length=10000, scalar_reward = False, seed=None, render_mode = None):
  ...
  def reset(self,
    ...
    self._unlocked = set()
    self.last_inventory = self._player.inventory.copy()
    self.last_achievements =  self._player.achievements.copy()
  ...
  def step(self, action):
    ...
    reward = []
    reward.append((self._player.health - self._last_health) / 10)
    self._last_health = self._player.health
    unlocked = {
    ...
    reward.extend([x - y for x, y in zip(self._player.inventory.copy().values(), self.last_inventory.values())])
    reward.pop(2)
    reward.extend([x - y for x, y in zip(self._player.achievements.copy().values(), self.last_achievements.values())])
    ...
    if self.scalar_reward:
      return obs, sum(reward), done, False, info
    else:
